...
private:
    void testCompareAndReset()
    {
        ...
        int value = 42;
        expect (gin::compareAndReset (value, 42) == true, "Should return true when values match");
        expect (value == 0, "Value should be reset to 0");

        value = 10;
        expect (gin::compareAndReset (value, 5) == false, "Should return false when values don't match");
        expect (value == 10, "Value should not be reset when comparison fails");

        float fvalue = 3.14f;
        expect (gin::compareAndReset (fvalue, 3.14f) == true, "Should work with floats");
    ...
    void testCalculateRMS()
    {
        ...
        // Test silence
        float rms = gin::calculateRMS (buffer, 0);
        ...
        rms = gin::calculateRMS (buffer.getReadPointer (0), 4);
    ...
    void testCalculateMedian()
    {
        ...
        expectWithinAbsoluteError (gin::calculateMedian (oddArray), 3.0f, 0.0001f,
                                   "Median of {5,1,3,2,4} should be 3");

        // Test with even number of elements
        juce::Array<float> evenArray = { 1.0f, 2.0f, 3.0f, 4.0f };
        expectWithinAbsoluteError (gin::calculateMedian (evenArray), 2.5f, 0.0001f,
                                   "Median of {1,2,3,4} should be 2.5");

        // Test with single element
        juce::Array<float> singleArray = { 42.0f };
        expectWithinAbsoluteError (gin::calculateMedian (singleArray), 42.0f, 0.0001f,
                                   "Median of single element should be that element");

        // Test with duplicates
        juce::Array<float> duplicateArray = { 1.0f, 2.0f, 2.0f, 2.0f, 3.0f };
        expectWithinAbsoluteError (gin::calculateMedian (duplicateArray), 2.0f, 0.0001f,
    ...
    void testRollingAverage()
    {
        ...
        gin::RollingAverage<float> avg (4);
    ...
    void testPerlinNoise()
    {
        ...
        gin::PerlinNoise noise1 (123);
        gin::PerlinNoise noise2 (456);
