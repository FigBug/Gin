...
def remove_gin_namespaces(source):
    """Return a string of source code with any gin namespaces removed.
    """
    namespace_regex = re.compile(r"\s+namespace\s+gin\s*{")
...
def add_doxygen_group(path, group_name):
    """Add a Doxygen group to the file at 'path'.
       ...
       The addition of gin namespacing code to all of the source files breaks
       backwards compatibility by changing the doc URLs, so we need to remove
       the namespaces.
    """

    filename = os.path.basename(path)
    if re.match(r"^gin.*\.(h|dox)", filename):
...
if __name__ == "__main__":
    ...
    # Get the list of gin modules to include.
    if args.subdirs:
        gin_modules = args.subdirs.split(",")
    else:
        gin_modules = []
        for item in os.listdir(args.source_dir):
            if os.path.isdir(os.path.join(args.source_dir, item)):
                gin_modules.append(item)

    # Copy the gin modules to the temporary directory, and process the source
    # files.
    module_definitions = []
    for module_name in gin_modules:
    ...
    with open(os.path.join(args.dest_dir, "gin_modules.dox"), "w") as f:
