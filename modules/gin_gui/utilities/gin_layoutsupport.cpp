...
static juce::File getLayoutFile (  const juce::String& shortCode, const juce::String& name )
{
	static juce::Array<juce::File> files;

	if ( files.isEmpty () )
	{
		// The environment-variable "{shortCode}_RESOURCES" has to point to the projects' "Assets" folder
		auto env = juce::SystemStats::getEnvironmentVariable ( shortCode.toUpperCase () + "_RESOURCES", "" );
		if ( env.isEmpty () )
		{
			jassertfalse;
			return {};
		...
		auto sourceDir = juce::File ( env ).getChildFile ( "Compiled/layouts" );
		...
		files = sourceDir.findChildFiles ( juce::File::findFiles, true, "*.json", juce::File::FollowSymlinks::noCycles );

		// Make sure there are no duplicate file names, names must be unique, even in different folders
		{
...
static std::string getComponentID ( juce::Component& c )
{
...
static juce::String getParentPath ( juce::String in )
{
	...
	if ( lastSlash > 0 )	return in.substring ( 0, lastSlash );
...
std::string ComponentMap::getComponentPath ( juce::Component& c )
{
...
juce::Component* ComponentMap::findComponent ( const std::string& path )
{
...
void ConstantsStack::set ( const std::string& name, double value )
{
	constants.push_back ( { name, value } );
}

std::optional<double> ConstantsStack::get ( const std::string& name )
{
...
Parser::Parser ( const juce::String equation )
{
	try
	{
		parser.addFunction ( "getX",  [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getX (); 						return 0; } );
		parser.addFunction ( "getY",  [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getY (); 						return 0; } );
		parser.addFunction ( "getW",  [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getWidth (); 					return 0; } );
		parser.addFunction ( "getH",  [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getHeight ();					return 0; } );
		parser.addFunction ( "getR",  [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getRight ();					return 0; } );
		parser.addFunction ( "getB",  [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getBottom ();					return 0; } );
		parser.addFunction ( "getCX", [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getBounds().getCentreX (); 	return 0; } );
		parser.addFunction ( "getCY", [ this ] ( int, const juce::String& id ) { if ( auto c = getComp ( id ) ) return c->getBounds().getCentreY (); 	return 0; } );
...
{
	if ( auto foundComp = compMap->findComponent ( cid.toStdString () ) )
		return foundComp;
	...
	if ( curComponent != nullptr )
	{
		...
		if ( auto foundComp = compMap->findComponent ( fullpath.toStdString () ) )
			return foundComp;
	}
...
void LayoutSupport::setLayoutInternal ( const juce::Array<JsonFile>& files )
{
	...
	// Find all the base level constants
	//
	compMap.createMap ( [] ( juce::Component& c )	{	return ! c.getProperties ().getWithDefault ( "usesLayoutSupport", false );	} );
...
void LayoutSupport::setConstant ( const std::string& name, int value )
{
	constants.set ( name, value );
}

void LayoutSupport::setComponentsLayout ( const std::string& currentPath,
	...
	for ( auto i = 0; const auto& component : getAllProperties ( components ) )
	{
		...
		if ( component.hasProperty ( "id" ) )
		{
			...
			for ( auto idIdx = 0; auto& id : ids )
			{
				...
				//
				if ( component.hasProperty ( "factory" ) && compMap.findComponent ( currentPath + "/" + id.toStdString () ) == nullptr )
				{
					if ( componentFactory )
					{
				...
				if ( component.hasProperty ( "macro" ) )
					prevComponent = setPosition ( currentPath, id.toStdString (), idIdx++, expandMacro ( component ) );
				else
					prevComponent = setPosition ( currentPath, id.toStdString (), idIdx++, component );
...
LayoutSupport::Bounds LayoutSupport::getBounds ( int idIdx, const juce::var& component )
{
	...
	if ( hasSize && component.hasProperty ( "cx" ) )	x = parse ( component[ "cx" ], idIdx ) - w / 2;
	if ( hasSize && component.hasProperty ( "cy" ) )	y = parse ( component[ "cy" ], idIdx ) - h / 2;
	if ( hasSize && component.hasProperty ( "r" ) )	x = parse ( component[ "r" ], idIdx ) - w;
	if ( hasSize && component.hasProperty ( "b" ) )	y = parse ( component[ "b" ], idIdx ) - h;
...
juce::var LayoutSupport::expandMacro ( const juce::var& component_ )
{
	...
	auto itr = macros.find ( function.toStdString () );
...
juce::Component* LayoutSupport::setPosition ( const std::string& currentPath,
											  const std::string& id,
											  int idIdx,
											  const juce::var& component )
{
	...
	if ( auto foundComp = compMap.findComponent ( path ) )
	{
		...
		if ( bounds.hasPosition )	curComponent->setTopLeftPosition ( bounds.x, bounds.y );
		if ( bounds.hasSize )		curComponent->setSize ( bounds.w, bounds.h );
...
void LayoutSupport::setGridPositions ( const std::string& currentPath, const juce::var& component )
{
	//
	// get ids
	//
	...
	for ( auto id : ids )
	{
		...
		if ( auto foundComp = compMap.findComponent ( path.toStdString () ) )
