 ...
 *
 * gin amalgamation override: disabled. Several library .c files (ecp.c,
 * ecjpake.c) define `static int self_test_rng(...)` under this guard; those
 * statics collide when multiple .c files share one translation unit (which
 * is what gin_network_mbed1.c etc. do). We don't run the in-tree self tests
 * at runtime, so leaving this off for the amalgamation build is the right
 * trade.
 */
 ...
 *
 * gin amalgamation override: disabled. LMS depends on PSA Crypto which we've
 * disabled (see MBEDTLS_PSA_CRYPTO_C). No first-party code uses LMS.
 */
